        ...
        for (size_t i = 0; i < operation_decoder->get_input_size(); ++i) {
            ...
            if (m_tensor_places.count(name) == 0) {
                ...
                if (auto data = place->get_data()) {
                    ...
                    else if(place->get_element_type() == element::i8){
                        std::cout << "----------Creating weights as params-----------" << std::endl;
                        input_idx++;
                        std::stringstream sstm;
                        if (name.find("transformer_feed_forward") != std::string::npos || name.find("softmax_feed_forward") != std::string::npos){
                             //compress int4 weights and write into bin file
                            int size_int4 = place->get_size()/2;
                            ...
                            sstm << "w_i8_" << input_idx << ".bin";
                            std::cout << "file name: " << sstm.str() << std::endl;
                            std::ofstream bin_file(sstm.str(), std::ofstream::binary);
                            bin_file.write((char*)data,sizeof(int8_t) * size);
                        }
